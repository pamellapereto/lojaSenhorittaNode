...
con.query("select * from carrinho where idusuario=?",[req.params.id],(erro,result)=>{
        if(erro){
                return res.status(500).send({output:`Erro ao tentar carregar o carrinho -> ${erro}`});
        }
        res.status(200).send({output:result});
});
...
app.put("/api/usuarios/carrinho/:id",(req,res)=>{
        con.query("update carrinho set ? where idcarrinho=?",[req.body,req.params.id],(erro, result)=>{
        if(erro){
        return res.status(500).send({output:`Não foi possível atualizar o carrinho -> ${erro}`});
        }
        res.status(200).send({output:result});
